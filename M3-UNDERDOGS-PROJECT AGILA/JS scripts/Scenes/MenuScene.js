class MenuScene extends Phaser.Scene{
    ...
    create(){
        ...
        let playButton = this.add.image(640,300, 'Play').setScale(2);
        playButton.setInteractive();
        playButton.on('pointerdown',() => {this.scene.start('CutScene1')
    });
    let creditButton = this.add.image(600,600, 'Credits').setScale(1);
    creditButton.setInteractive();
    creditButton.on('pointerdown',() => {this.scene.start('CreditScene')});
    
    let instructionButton = this.add.image(150,600, 'help').setScale(1);
